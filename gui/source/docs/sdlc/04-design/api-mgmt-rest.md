...
> **TL;DR:** Three endpoints on port 3001 (loopback by default): `/healthz` (liveness), `/readyz` (readiness), `/metrics` (Prometheus). Read-only. No application-level auth (loopback IS the access control). Future operator-control endpoints (`/admin/...`) land in M11 with auth required when bound non-loopback. Open to k8s / Docker health probes; closed to public traffic.
...
**Response (M0 baseline):**

```json
{ "status": "ready" }
```

**Response (M1+, planned with rich state):**
