...
> **TL;DR:** 12 tables in Postgres (or pglite in dev). Drizzle-typed schema. Versioned migrations in `src/storage/migrations/`. ER diagram below; per-table fields documented inline. Audit chain (`auditEntries`) is append-only; everything else is normal CRUD with tenant scoping.
