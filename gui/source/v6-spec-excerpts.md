# v6 Spec — Sections most relevant for the control-plane GUI design

> The full v6 spec is ~2,700 lines. This file extracts the sections most relevant for designing an operator-facing status monitor and control plane: state machine, architecture, MCP surface, transport, observability, milestones, security, lethal trifecta. For canonical / complete reading: see the original spec in the repo root.

Sections extracted:
- §1 Mission, §3 Assumptions, §4 Non-Goals (orientation)
- §5 Core User Flow
- §6 Project State Machine (the state model the GUI surfaces)
- §7 High-Level Architecture (where the GUI fits)
- §10 Domain Model
- §14 MCP Surface Area (tools the GUI references)
- §17 Readiness Rubric (one of the GUI's primary surfacings)
- §18 Write Safety + Idempotency (relevant for approval gates)
- §22 Transport and Deployment (mgmt port = the GUI's API)
- §24 Job Queue and Async Workflows (provisioning queue)
- §27 Observability and Telemetry (counters + traces the GUI consumes)
- §28 Implementation Milestones (status surfacing)
- §30 Security Requirements (audit chain — the GUI's audit viewer)
- §38 Cross-cutting Safety Concepts (lethal trifecta — approval queue)
