...
Hand-authored Claude Code-specific guidance. `scripts/syncAgentConfigs.ts` prepends the AGENTS.md content to this addendum and writes the combined result to `CLAUDE.md`.
...
- **Read large files in chunks.** F-207 from [`docs/partners/project-foundation-workbench.md`](docs/partners/project-foundation-workbench.md) is about avoiding context-window bloat while reading. Use `offset` + `limit` for large files; split files when it improves reviewability or ownership.
