...
> **TL;DR:** State-machine coordinators and deterministic workflow helpers for multi-step operations. Intake → blueprint, blueprint → plan → execute, readiness, handoff, webhook ingestion, work classification, agent recommendation, and content quality scoring. Workflow code is pure or repository-backed through injected dependencies; durable state is persisted through storage and audited by tool wrappers.
...
- Deterministic work classification and agent recommendation helpers for developer assignment.
- Deterministic content quality scoring for project and artifact trustworthiness reports.
...
| `classifyWorkItem` | function | Classify a story/task/Jira issue into work type, tags, risk, and confidence |
| `recommendAgentsForWork` | function | Rank live and persisted agents against a work classification |
| `scoreProjectContentQuality` | function | Score project/artifact content against the deterministic trustworthiness rubric |
...
### Work classification and agent recommendation

1. Admin tool resolves a `WorkRef` to a blueprint story/task or Jira issue.
2. `classifyWorkItem` scans title, description, acceptance criteria, and labels for known work classes.
3. Output includes `workType`, `skillTags`, `riskLevel`, `confidence`, and a short explanation.
4. `recommendAgentsForWork` scores live MCP sessions and persisted session profiles against classification tags and work type.
5. Assignment writes are handled outside the workflow helper by `admin.agent.work.assign`, which persists `WorkAssignmentRecord` and appends audit evidence.

### Content quality scoring

1. Admin tool resolves the project or artifact reference.
2. `scoreProjectContentQuality` runs deterministic checks for completeness, traceability, source grounding, freshness, consistency, and actionability.
3. The tool persists a `ContentQualityReport` with score, grade, findings, recommendations, and `llmCritique.status`.
4. LLM critique is currently reported as `unavailable` unless a future provider is explicitly configured.
...
- **Ambiguous work item** — classification returns `unknown` with lower confidence; assignment remains a human decision.
- **Weak project content** — quality report surfaces warnings/failures; it does not block by itself until a policy gate is added.
...
| Work classification and recommendation | `tests/integration/admin/roleWorkflowTools.test.ts` |
| Content quality scorer | `tests/unit/workflows/contentQualityScorer.test.ts`, `tests/integration/admin/roleWorkflowTools.test.ts` |
...
- Role workflow extension: work classification and content quality scoring support the control-plane Requirements Assist, Agent Assignment, and Quality panels.

## Linked artifacts

- **Spec:** v6 §5 (core flow), §6 (state machine), §18 (write safety), §23 (sampling)
- **Code:** `src/workflows/`, `src/domain/projectState.ts`
- **Role workflow UI:** [`control-plane-ui/role-workflows.md`](control-plane-ui/role-workflows.md)
