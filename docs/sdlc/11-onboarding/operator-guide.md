...
> **TL;DR:** You're running atl-mcp in production. Here's what you need to know to do that confidently. This guide complements the [`runbook.md`](../08-operations/runbook.md) (symptom-organized incident response) and includes the loopback `/ui/` control plane workflows for role-aware project initiation, agent assignment, and content quality scoring.
...
Open the management UI at `/ui/` on the loopback management API. The top navigation includes a role selector for Customer, Product Owner, Scrum Master, Developer, DevOps Engineer, and Operator. The selector changes presentation only; it does not grant access or hide data as a security boundary.

Common steady-state workflows:

1. **Product initiation:** open `#/requirements-assist` or the Projects page assist panel, enter a project description, add brief excerpts, preview requirements, create intake, generate blueprint, and preview Jira nodes.
2. **Developer assignment:** open `#/agent-assignment` or a project detail Assignments tab, select a story/task/Jira issue, review classification and recommendations, assign an agent with a reason, and confirm assignment history.
3. **Agent catalog review:** open Sessions/Agents to view connected sessions plus the static catalog of analyzed code-engine and ops-engine agent roles before assigning work.
4. **Content quality review:** score project or artifact content from Projects, Project Detail Quality tab, or Agent Assignment; treat low scores as review signals before provisioning or handoff.
5. **Operational control:** use Dashboard, Jobs, Policy, Providers, Audit, Secrets, Migrations, SLO, Capacity, and DR for the traditional operator workflows.
