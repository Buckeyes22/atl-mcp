...
> **TL;DR:** 23 root typed entities in `src/domain/`, plus supporting subtypes for work classification and content quality findings. Distinguishes "domain types" (in-memory, business logic) from "schema" (DB tables). Some domain types map 1:1 to tables; others are sub-aggregates serialized inside JSONB. Round-trip serialization is unit-tested.
...
| `AgentMemoryEntry` | `agentMemory.ts` | Project-scoped durable agent memory |
...
| `WorkAssignmentRecord` | `workAssignment.ts` | Persisted developer assignment intent for a story/task/Jira issue |
| `ContentQualityReport` | `contentQuality.ts` | Persisted score, grade, findings, recommendations, and critique status for project/artifact content |

That's 23 root types (some are factored into multiple files). Supporting subtypes include `WorkRef`, `WorkClassification`, `AgentRecommendation`, `ContentQualityFinding`, and `ContentQualityLlmCritique`.
...
classDiagram
    ...
    ContextPack *-- "0..*" AgentMemoryEntry

    WorkAssignmentRecord *-- WorkRef
    WorkAssignmentRecord *-- WorkClassification
    WorkAssignmentRecord *-- "0..*" AgentRecommendation

    ContentQualityReport *-- ArtifactRef
    ContentQualityReport *-- "0..*" ContentQualityFinding
    ContentQualityReport *-- ContentQualityLlmCritique
```
...
```typescript
interface WorkAssignmentRecord {
  id: string
  tenantId: string
  projectId: string
  workRef: WorkRef
  classification: WorkClassification
  recommendedAgents: readonly AgentRecommendation[]
  assignedAgentId?: string
  assignedBy?: string
  status: "suggested" | "assigned" | "blocked" | "completed"
}
```

Represents human assignment intent for developer work. It does not mean the build agent has executed the work; execution state remains in jobs, webhooks, and downstream workflow records.
...
interface ContentQualityReport {
  ...
  score: number
  grade: "A" | "B" | "C" | "D"
  findings: readonly ContentQualityFinding[]
  recommendations: readonly string[]
}
```

Scores trustworthiness of requirements, blueprint, Jira, Confluence, or handoff content. Current scoring is deterministic; `llmCritique` is present with `status: "unavailable"` unless a future critique provider is configured.
