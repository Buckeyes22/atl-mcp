...
| **PRIVATE** | Customer-confidential; never crosses the trust boundary outward without explicit authorization. | Customer source code, project profile prose content, requirements descriptions, issue body text, agent memory text derived from project work |
...
| Requirements Assist descriptions and brief excerpts | PRIVATE |
| Work assignment payloads and reasons | INTERNAL by default; PRIVATE when they include customer requirement text |
| Content quality findings and recommendations | PRIVATE when derived from project content |
| Agent role catalog metadata | INTERNAL |
| Browser-local `roleLens` preference | INTERNAL |
...
## Role Workflow Classification

Role workflow features add these classification rules:

| Surface | Classification rule |
|---|---|
| Requirements Assist description | PRIVATE because it is project profile prose and may contain customer-confidential business intent. |
| Uploaded or pasted brief text | PRIVATE unless source metadata marks it higher. The browser sends only extracted text/metadata to admin tools and must not store brief contents in tweak storage. |
| Generated requirements, epics, stories, and Jira previews | PRIVATE because they derive from requirements and brief content. |
| Work classification output | INTERNAL for taxonomy fields; PRIVATE for embedded story/task descriptions or acceptance criteria. |
| Agent recommendations | INTERNAL when based on session/profile metadata; do not include project prose unless required for explanation. |
| Assignment reason | INTERNAL by default; treat as PRIVATE if the user writes customer details into the reason. |
| Content quality report | PRIVATE because findings and recommendations can quote or summarize weaknesses in customer project content. |
| Agent role catalog | INTERNAL static operational metadata. |
| `roleLens` | INTERNAL local preference; it is not an entitlement, ACL, redaction rule, or privacy boundary. |
| `operatorBadge` | INTERNAL identity metadata; appears in audit contexts where writes are attributed. |
