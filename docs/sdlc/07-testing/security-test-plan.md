...
| T-1109 | Poison agent memory | scoped memory + audit + forget | Existing in `tests/unit/workflows/agentMemoryWorkflow.test.ts` |
| T-1110 | Treat role lens as authorization | presentation-only role lens | Existing in `tests/unit/controlPlaneProvisionUi.test.ts`, `tests/unit/controlSurfaceModel.test.ts` |
| T-1111 | Leak assisted requirements content | no brief content in local tweaks | Source-level UI coverage in `tests/unit/controlPlaneProvisionUi.test.ts`; dedicated negative test recommended |
| T-1112 | Trust low-quality assisted content | quality score findings | Existing in `tests/unit/workflows/contentQualityScorer.test.ts`, `tests/integration/admin/roleWorkflowTools.test.ts` |
...
- T-1111 needs a dedicated negative test that confirms Requirements Assist brief text is not persisted into `CPTweaksProvider` storage.
...
- **Role workflow tests:** `tests/unit/controlPlaneProvisionUi.test.ts`, `tests/unit/controlSurfaceModel.test.ts`, `tests/unit/workflows/contentQualityScorer.test.ts`, `tests/integration/admin/roleWorkflowTools.test.ts`
