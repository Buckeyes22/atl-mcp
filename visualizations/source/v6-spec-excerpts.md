# v6 Spec — Extracted Sections (for visualization context)

> The full v6 spec is ~2,700 lines. This file extracts the sections most-referenced by the visualizations to be created. For canonical / complete reading: see the original spec in the repo root.

Sections extracted:
- §0 v6 Review Summary
- §1 Mission, §2 Strategic Design Decision, §3 Assumptions, §4 Non-Goals, §5 Core User Flow, §6 Project State Machine, §7 High-Level Architecture
- §10 Domain Model
- §14 MCP Surface Area
...
- §31 Testing Strategy
- §38 Cross-cutting Safety Concepts (lethal trifecta + ACL)
...
- **Memory layer: not adopted in v1.** Considered: hindsight three-op (retain/recall/reflect), mengram 3-type memory, Memory Bank `.md` files. Reason for default: v1 scope is project orchestration, not agent memory; persistent memory belongs to consuming agents, not the orchestrator. References preserved in §25 for future work.
...
The plan does not adopt persistent agent memory in v1. Consumers that want cross-session memory can layer `@alioshr/memory-bank-mcp` (or equivalent) above the orchestrator using the **Memory Bank pattern**: 6 persistent `.md` files (`projectbrief.md`, `productContext.md`, `systemPatterns.md`, `techContext.md`, `activeContext.md`, `progress.md`) maintained outside the orchestrator's context-pack scope (from agentic-coding-handbook, see §40 F-103).
The plan does not adopt persistent agent memory in v1. Consumers that want cross-session memory can layer `@alioshr/memory-bank-mcp` (or equivalent) above the orchestrator using the **Memory Bank pattern**: 6 persistent `.md` files (`projectbrief.md`, `productContext.md`, `systemPatterns.md`, `techContext.md`, `activeContext.md`, `progress.md`) maintained outside the orchestrator's context-pack scope (from agentic-coding-handbook, see §40 F-103).
