...
- **Keep [`docs/documentation-catalog.md`](docs/documentation-catalog.md) updated.** It's the canonical doc index. Any change that creates, removes, renames, or substantially modifies a documentation file (under `docs/`, `visualizations/`, `gui/`, or any future doc surface) MUST update the catalog in the same change. The per-surface tables, top-level summary counts, embedded-visualizations index, and persona-routed reading orders should all stay current. Don't queue catalog updates for "later."
